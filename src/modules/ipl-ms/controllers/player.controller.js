...
export async function handleCreatePlayer(req, res) {
  try {
    ...
    res
      .status(201)
      .json(ApiResponse(201, player, "Player created successfully!"));
  } catch (err) {
...
export async function handleGetAllPlayer(req, res) {
  try {
    ...
    res
      .status(200)
      .json(ApiResponse(200, player, "Players fetched successfully!"));
  } catch (err) {
    res.status(500).json(ApiError(500, err.message));
  }
}
...
export async function handleGetPlayerById(req, res) {
  try {
    const player = await createPlayer(req.params.id);
    res
      .status(200)
      .json(ApiResponse(200, player, "Player fetched successfully!"));
  } catch (err) {
    res.status(500).json(ApiError(500, err.message));
  }
}
...
export async function handleDeletePlayer(req, res) {
  try {
    ...
    res
      .status(200)
      .json(ApiResponse(200, player, "Player deleted successfully!"));
  } catch (err) {
    res.status(500).json(ApiError(500, err.message));
  }
}
